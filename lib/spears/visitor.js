...
var Visitor = function (options) {
  ...
  // Provide a default object for options
  if (!options) {
    options = {}
  }
  ...
  this.serialFeaturesToRun   = Spears.Cucumber.Ast.Features();
  ...
  this.visitFeature = function(feature, iterate) {
    ...
    var uri     = feature.getUri();
    var line    = feature.getLine();

    buildCurrFeature = function () {
      return Spears.Cucumber.Ast.Feature(keyword, name, desc, uri, line);
    };
  ...
  this.visitScenario = function (scenario, iterate) {
    ...
    for (i in tags) {
      if (!self.options.serial && tags[i].getName() === Enum.TAG_PARALLEL_OK) {
        ...
        if (currBackground !== null) feature.addBackground(currBackground);
    ...
    if (!tags[i] || !isParallel && tags[i].getName() !== Enum.TAG_IGNORE_IF_PARALLEL) {
      ...
      if (!currSerialFeature) {
        ...
        if (currBackground !== null) currSerialFeature.addBackground(currBackground);
        self.serialFeaturesToRun.addFeature(currSerialFeature);
      }
