...
var Visitor = function (environmentCount) {
  ...
  var self                  = this
    ...
    , buildCurrParallelFeature = null // There is one feature per scenario for parallel
  ...
  this.visitFeature = function(feature, iterate) {
    ...
    self.serialFeaturesToRun[self.feature_idx]   = [Spears.Cucumber.Ast.Features()];

    buildCurrParallelFeature = function () {
    ...
    currSerialFeature = Spears.Cucumber.Ast.Feature(
      feature.getKeyword(),
      feature.getName(),
      feature.getDescription(),
      feature.getUri(),
    ...
    self.serialFeaturesToRun[self.feature_idx][0].addFeature(currSerialFeature);
  ...
  this.visitScenario = function (scenario, iterate) {
    ...
    for (i in tags) {
      if (tags[i].getName() === Enum.TAG_PARALLEL_OK) {
        ...
        feature = buildCurrParallelFeature();
    ...
    if (!isParallel && tags[i].getName() !== Enum.TAG_IGNORE_IF_PARALLEL) {
