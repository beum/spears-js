 ...
 */
var ParallelFormatter = function (options, listener) {
  if (false === (this instanceof ParallelFormatter)) {
    return new ParallelFormatter(options, listener);
  }
  ...
  self.handleBeforeFeatureEvent = function handleBeforeFeatureEvent(event, callback) {
    ...
    self.feature = source;
    if (options && options.isSerial) listener.handleBeforeFeatureEvent(source);
    callback();
  };
  ...
  self.handleBeforeScenarioEvent = function handleBeforeScenarioEvent(event, callback) {
    ...
    if (options && options.isSerial) listener.handleBeforeScenarioEvent(source);
    callback();
  };
  ...
  self.handleStepResultEvent = function handleStepResultEvent(event, callback) {
    ...
    if (options && options.isSerial) listener.handleStepResultEvent(output);
    callback();
  };
