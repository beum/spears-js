...
module.exports = (function () {
  ...
  self.handleFeatureComplete = function (PF) {
    var scenarios, steps, i, j;
    log("");
    log(boldColors.azure + PF.feature + colors.clear);


    scenarios = PF.scenarios;
    for (j in scenarios) {
      log("  " + boldColors.blue + scenarios[j].title + colors.clear);
      steps = scenarios[j].steps;

      for (i in steps) {
        if (steps[i].type === 'pending') {
          log("    " + colors.orange + steps[i].text + colors.clear);
        } else if (steps[i].type === 'undefined') {
          log("    " + boldColors.orange + steps[i].text + colors.clear);
        } else if (steps[i].type === 'skipped') {
          log("    " + effects.strikethrough + steps[i].text + colors.clear);
        } else if (steps[i].type === 'failed') {
          log("    " + boldColors.red + steps[i].text + colors.clear);
          log(colors.red + steps[i].failureText + colors.clear);
        } else {
          log("    " + colors.green + steps[i].text + colors.clear);
        }
      }
